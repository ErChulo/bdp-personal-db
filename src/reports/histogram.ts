...
export function renderHistogramSvg(bins: Bin[]): string {
  ...
  const rect = bins
    ...
    .join('');
  return `<svg viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg" style="max-width:100%;font-family:monospace">${rect}<text x="0" y="14" fill="var(--fg-muted)" font-size="10">n=${bins.reduce((s, b) => s + b.count, 0)}</text></svg>`;
}
